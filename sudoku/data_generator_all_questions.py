...
class SudokuDataGeneratorAllQuestions:
    ...
    def generate_dataset(self, num_samples: int, output_dir: str):
        ...
        for state in sudoku_states:
            ...
            # 为每个数独状态生成所有问题类型
            for qa_type in question_types:
                qa_data = self.generate_question(board, solved_board, qa_type)
                
                if(qa_data["question_id"] == 1 or qa_data["question_id"] == 5):
                    data_entry = {
                        "data_id": f"sudoku-{data_id_counter:05d}",
...
def main():
    parser = argparse.ArgumentParser(description='Generate Sudoku dataset with all question types per state')
    parser.add_argument('num_samples', type=int, help='Number of sudoku states to generate')
    parser.add_argument('--output_dir', type=str, default="sudoku_dataset_all_questions", 
                      help='Output directory for the dataset')
    
    args = parser.parse_args()
    
    # 初始化为9x9，在生成过程中会根据需要切换大小
    generator = SudokuDataGeneratorAllQuestions(9)
    generator.generate_dataset(args.num_samples, args.output_dir)
    print(f"Successfully generated {args.num_samples} sudoku states with all question types in {args.output_dir}")
